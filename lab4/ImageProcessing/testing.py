...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "inputs",
        nargs="*",
        help=("Введите количество изображений (целое, по умолчанию 1). ")
        )
    p.add_argument(
        "-o", "--output",
        default=None,
        help="Путь/каталог для сохранения.",
    )
    return p
...
def run_api_flow(method: str, count_str: str | None, outdir: str | None) -> None:
    ...
    try:
        ...
        cats = processor.download_data(limit=count)
        if cats.size == 0:
